...
<details>
     <summary> Voir plus d'information sur la gestion des dépendances </summary>
<br>

**Un outil de gestion des dépendances vous permet de garder une liste à jour de vos dépendance** et favorise la portabilité de vos application, tout en évitant de stocker du code inutile dans vos répertoires de données. 

Par exemple, en utilisant NPM

```bash
// Creation du package.json
npm init
// (suivre les étapes proposée)
// Installation d'une dépendance (note: le save est maintenant par défaut)
npm install bcrypt --save
// Lecture de la liste des dépendances
vi package.json
// Test de l'installation, suppression des modules et re-installation sur la base du package.json
rm -rf node_modules/
npm install
```
</details>
